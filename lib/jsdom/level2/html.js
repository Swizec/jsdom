...
core.resourceLoader = {
  load: function(element, href, callback) {
    ...
    if (ownerImplementation.hasFeature('FetchExternalResources', element.tagName.toLowerCase())) {
      ...
      if (url.hostname) {
	  if (loadExternal) this.download(url, this.enqueue(element, callback));
      }
      else {
	var file = this.resolve(element.ownerDocument, url.pathname);
	if (file.hostname) {
	    if (loadExternal) this.download(file, this.enqueue(element, callback));
	}else{
  ...
  resolve: function(document, path) {
    if (['/','.'].indexOf(path.charAt(0)) !== -1) {
      ...
      if (document._documentRoot.hostname ) {
	path = URL.parse(URL.resolve(document._documentRoot, path));
      }else{
